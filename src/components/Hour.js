...
const Card = ({ weather }) => {
  ...
  const hours = hourConditions.map((hour, index) => {
    ...
    return (
      <div key={index} className="flex flex-col items-center shrink-0 text-sm pt-2 border-x-2 snap-start">
  ...
  return (
    <div className="bg-white h-1/6 w-5/6 mx-auto rounded-3xl shadow-xl overflow-hidden">
      <div className="flex overflow-x-auto scroll-smooth h-full curso pointer snap-x">{hours}</div>
    </div>
  )
};

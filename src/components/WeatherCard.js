...
const WeatherCard = ({ index, weather, day }) => {
  ...
  useEffect(() => { setTemp(conditions.celcius) }, [weather]);
  ...
  const forecastConditions = {
    ...
    rain: 'Rain: ' + weather.forecast.forecastday[index].day.daily_will_it_rain + 'mm',
    humidity: 'Humidity: ' + weather.forecast.forecastday[index].day.avghumidity + '%',
    wind: 'Wind: ' + weather.forecast.forecastday[index].day.avgvis_km + ' km/h',
  ...
  return (
    <div className="flex justify-between bg-white h-1/6 w-5/6 mx-auto rounded-3xl shadow-xl overflow-hidden">
      ...
      <div className="text-center self-center ">
        ...
        <span className="cursor-pointer" onClick={() => { setTemp(conditions.celcius) }}>⁰C</span>
        <span> | </span>
        <span className="cursor-pointer" onClick={() => { setTemp(conditions.farenheit) }}>⁰F</span>
      </div>
